import { initializeAdMob } from './admob_mock.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function placeNumber(num) {
        ...
        if (num === 0) {
            ...
            addToHistory(`Number cleared at pos (${row + 1}, ${col + 1})`, 'default');
        }
        else {
            if (Sudoku.isValidMove(state.board, row, col, num)) {
                ...
                addToHistory(`Number ${num} placed at pos (${row + 1}, ${col + 1})`, 'success');
                ...
                addToHistory(`Error: ${num} is incorrect at (${row + 1}, ${col + 1}) -50 points`, 'error');
    ...
    function showLevelCompleteScreen() {
        ...
        messageTitle.textContent = 'Level Completed';
        messageText.innerHTML = `Congratulations! You passed Level ${state.currentLevel}.<br>Total Score: <strong>${state.totalScore}</strong>`;
        messageButton.textContent = 'Next Level';
        messageTitle.className = 'level-complete-title';
        messageContainer.style.display = 'block';
        createFlares();

        addToHistory(`Level ${state.currentLevel} completed! Score: ${state.totalScore}`, 'success');
    ...
    function showGameCompleteScreen() {
        ...
        messageText.innerHTML = `You are a true Sudoku Master!<br>Final Score: <strong>${state.totalScore}</strong>`;
        messageButton.textContent = 'Play Again';
        messageTitle.className = 'game-complete-title';

        addToHistory(`Game completed! Final Score: ${state.totalScore}`, 'success');

        const text = "CONGRATULATIONS    GAME    COMPLETED";
    ...
    function showGameOver() {
        ...
        addToHistory(`Game Over! Too many errors. Final Score: ${state.totalScore}`, 'error');

        showMessage('GAME OVER', `You made 5 errors. Final Score: ${state.totalScore}`, 'Play Again', () => {
    ...
    function startNewLevel() {
        ...
        addToHistory(`Started Level ${state.currentLevel}`, 'default');
    }

    function resetGame() {
        if (confirm('Are you sure you want to start a new game from Level 1? All progress will be lost.')) {
            stopTimer();
    ...
    function giveHint() {
        if (state.hints <= 0) {
            showMessage('No Hints', 'You have no hints left for this level.', 'OK');
            return;
        }
        ...
        addToHistory(`Hint used: Number ${correctValue} at (${r + 1}, ${c + 1}) -100 points`, 'hint');
